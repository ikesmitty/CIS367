class Cube {
    ...
    constructor (gl, size, div, col1, col2) {
        ...
        for(let j = 0; j <= div; j++) {
            for(let k = 0; k <= div; k++) {
                let x = (segLength * k) - halfSZ;
                let y = (segLength * (div - j)) - halfSZ;

                let point = vec3.fromValues(x, y, halfSZ);
        ...
        // Create the other 5 faces with these dimensions,
        let origin = vec3.create();

        // Rotate around the X axis for the 2 longitudinal sides
        for(let i = 0; i<2; i++) {
        ...
        // Push points to build the faces
        for(let i = 0; i <= 6; i++) {
